...
本游戏**默认＝纯规则引擎、完全离线、不联网、不接任何云端 AI**。系统层面保留**可插拔的本地 LLM 接口**（`LocalLLM` / `LLMProvider` / `LLMConfig` / `ResponseGenerator`）：规则引擎负责“想什么”，若你接入本地 llama.cpp，LLM 只负责“怎么说”。

- **默认**：走纯规则，无需任何模型/权限，直接可玩。
- **启用**：底栏「模型」按钮开关。若检测不到 Android 本地推理插件，自动回退纯规则，**不会报错、不会空回应**。
- 真正接模型需在手机端做 **Android 插件 + llama.cpp + GGUF**（见 `docs/Android_LLM_插件接入.md`），Godot 沙箱无法编译验证该部分，需真机测试。
